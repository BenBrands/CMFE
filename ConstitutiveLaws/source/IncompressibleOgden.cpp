...
template <int dim>
IncompressibleOgden<dim>::IncompressibleOgden(const Vector<double> alpha_,
...
template <int dim>
void
IncompressibleOgden<dim>::stress_S_i(double &S,
...
template <int dim>
void
IncompressibleOgden<dim>::stress_S(SymmetricTensor<2,dim> &tensor_S,
									  const SymmetricTensor<2,dim> &tensor_C) const
{
	(void)tensor_S;
	(void)tensor_C;

	Assert(false,ExcMessage("The function stress_S_iso is not callable for IncompressibleOgden"));
...
template <int dim>
void
IncompressibleOgden<dim>::material_tangent(SymmetricTensor<4,dim> &tangent,
									  	  	  const SymmetricTensor<2,dim> &tensor_C) const
{
	(void)tangent;
	(void)tensor_C;

	Assert(false,ExcMessage("The function material_tangent_iso is not callable for IncompressibleOgden"));
...
template <int dim>
void
IncompressibleOgden<dim>::stress_S_iso(SymmetricTensor<2,dim> &tensor_S,
		  	  	  	  	  	  			  const SymmetricTensor<2,dim> &tensor_C) const
{
...
template <int dim>
void
IncompressibleOgden<dim>::stress_S_vol(SymmetricTensor<2,dim> &tensor_S,
		  	  	  	  	  	  			  const SymmetricTensor<2,dim> &tensor_C) const
{
	 double J=1;
	...
	tensor_S=J*(B/alpha_vol)*std::sinh(alpha_vol*(J-1))*invert(tensor_C);
}


template <int dim>
void
IncompressibleOgden<dim>::material_tangent_iso(SymmetricTensor<4,dim> &tangent,
							  	  	  			  const SymmetricTensor<2,dim> &tensor_C) const
{
	 double J=1;
...
template <int dim>
void
IncompressibleOgden<dim>::material_tangent_vol(SymmetricTensor<4,dim> &tangent,
							  	  	  			  const SymmetricTensor<2,dim> &tensor_C) const
{
	 double J=1;
	...
	C_vol1*=((B/alpha_vol)*J*std::sinh(alpha_vol*(J-1))+std::pow(J,2)*B*std::cosh(alpha_vol*(J-1)));

	C_vol2=tensorproduct(C_inv,C_inv);

	C_vol2*=2*J*(B/alpha_vol)*std::sinh(alpha_vol*(J-1));
...
namespace boost
{
	namespace serialization
	{
		...
		inline void save_construct_data(Archive & ar,
										const ConstitutiveLaws::IncompressibleOgden<dim> *object_O,
		...
		inline void load_construct_data(Archive & ar,
										ConstitutiveLaws::IncompressibleOgden<dim> *object_O,
										const unsigned int /*version*/)
		{
			// retrieve data from archive required to construct new instance
			Vector<double> alpha, mu;
			double alpha_vol=0, B=0;
			ar >> alpha;
			ar >> mu;
			ar >> alpha_vol;
			ar >> B;
			...
			::new(object_O)ConstitutiveLaws::IncompressibleOgden<dim>(alpha,mu,alpha_vol,B);
		}
	}//namespace serialization
}//namespace boost


#include "IncompressibleOgden.inst"
